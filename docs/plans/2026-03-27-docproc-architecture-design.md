...
│   └── registry/      # NestJS + Next.js — gestión de tenants + Admin UI
├── packages/
│   ├── api-contracts/ # Zod schemas compartidos (core ↔ registry)
│   ├── ui/            # Componentes compartidos (review UI + admin UI)
│   └── typescript-config/
├── docker-compose.yml
└── turbo.json
```
...
## Migración desde el POC actual

1. Crear tag `v1.0-poc-vesprini` en el commit actual para preservar el estado funcional
2. Crear nuevo repo `docproc` con la estructura arriba
3. Vesprini pasa a ser el **primer tenant**: se configura `DocumentType: Remito` con los campos actuales
4. La app Vesprini (`poc-mrf`) se convierte en cliente del core via API key
5. La lógica de extracción actual (`claude-extraction.ts`) migra al core
